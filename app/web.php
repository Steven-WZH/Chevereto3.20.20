    ...
    $hook_before = function ($handler) {
        ...
        if($handler->request_array[0] == getSetting('route_image')) {
            ...
            if ($id !== false) {
                ...
                $userNotBanned = isset($image['user']['status']) ? $image['user']['status'] != 'banned' : false;
                if ($image && $image['is_approved'] && $userNotBanned && !in_array($image['album']['privacy'], array('private', 'custom'))) {
                    ...
                    $imageTitle = strip_tags($image['title']) ?: ($image_safe_html['name'] . '.' . $image['extension']) . ' hosted at ' . getSetting('website_name');
                    $handler::setVar('oembed', ['title' => $imageTitle, 'url' => $image['url_viewer']]);
                }
