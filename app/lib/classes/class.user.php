...
class User
{
    ...
    public static function getAlbums($var)
    {
        ...
        $children = [];
        $db = DB::getInstance();
        $db->query('SELECT * FROM ' . DB::getTable('albums') . ' WHERE album_user_id=:image_user_id ORDER BY album_parent_id ASC, album_name ASC');
    ...
    public static function fill(&$user)
    {
        ...
        if (isset($user['website']) && !filter_var($user['website'], FILTER_VALIDATE_URL)) {
            unset($user['website']);
        }
