...
function check_system_integrity()
{
    ...
    $imagetype_fail = 'image type support is not enabled.';
    $failed_formats = [];
    if($image_library === 'imagick') {
        ...
        foreach($mustHaveFormats as $format) {
            if(!in_array($format, $imageFormats)) {
                ...
                $install_errors[] = "Imagick $format $imagetype_fail";
            }
        }
        ...
        foreach($mustHaveFormats as $format) {
            if(!($imageTypes & constant("IMG_$format"))) {
                $failed_formats[] = $format;
                $install_errors[] = "GD $format $imagetype_fail";
            }
        }
    }
    if($failed_formats !== []) {
        chevereto_die(null, '<p>The '.$image_library.' image library in this provisioning lacks support for the following image format(s):</p><ul class="errors"><li>' . implode('</li><li>', $failed_formats) . '</li></ul><p>Check our documentation on <a href="https://chv.to/v3imglib" target="_blank">Image library requirements</a> to workaround this.</p>', "Missing $image_library format(s)");
    }
